...
def weekly_dates(data_dir: Path) -> list[date]:
    ...
    frame = pl.DataFrame({"date": sorted(set(dates))})
    return (
        frame.with_columns(pl.col("date").dt.strftime("%G-%V").alias("week"))
...
def collect(data_dir: Path, output: Path) -> dict[str, Any]:
    planned_dates = weekly_dates(data_dir)
    ...
    try:
        for index, day in enumerate(planned_dates, start=1):
            rows.extend(
                client.query(
                    "hk_hold",
            ...
            if index == 1 or index % 25 == 0 or index == len(planned_dates):
                print(
                    f"northbound_progress={index}/{len(planned_dates)} rows={len(rows)}",
                    flush=True,
                )
    finally:
        client.close()
    ...
    returned_dates = set(holdings["date"].unique().to_list())
    missing_dates = sorted(set(planned_dates) - returned_dates)
    invalid_values = holdings.filter(
        pl.col("holding_shares").is_null()
        | pl.col("holding_ratio_pct").is_null()
        | (pl.col("holding_shares") < 0)
        | (pl.col("holding_ratio_pct") < 0)
    ).height
    minimum_week_symbols = counts["symbols"].min() if counts.height else 0
    date_coverage = len(returned_dates) / len(planned_dates)
    checks = {
        "weekly_date_coverage_at_least_98pct": date_coverage >= 0.98,
        "symbol_date_unique": holdings.unique(["symbol", "date"]).height
        == holdings.height,
        "critical_values_valid": invalid_values == 0,
    ...
    payload = {
        "schema_version": "p0-northbound-weekly-holdings-v1",
        ...
        "counts": {
            ...
            "returned_weeks": len(returned_dates),

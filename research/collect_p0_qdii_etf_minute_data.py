"""Collect and phase-isolate QDII ETF minute data without evaluating returns."""
...
def normalize_minutes(rows: list[dict[str, Any]], symbol: str) -> pl.DataFrame:
    schema = {
        "symbol": pl.Utf8, "datetime": pl.Datetime, "open": pl.Float64,
...
def audit(
    ...
    daily_keys = daily.select("symbol", "date").unique()
    invalid = minutes.filter(
        (pl.col("open") <= 0)
    ...
    checks = {
        "frozen_symbols_exact": set(minutes["symbol"].unique().to_list()) == set(SYMBOLS),
        ...
        "all_minute_dates_have_daily_rows": minute_keys.join(
            daily_keys, on=["symbol", "date"], how="anti"
        ).is_empty(),
        "prices_and_activity_valid": invalid.is_empty(),
    }
    return {
        ...
        "counts": {
            ...
            "per_symbol_complete_days": {
                symbol: complete.filter(pl.col("symbol") == symbol).height for symbol in SYMBOLS
            },
        },
        "checks": checks,
    }
...
def collect(data_dir: Path, output: Path) -> dict[str, Any]:
    ...
    root = data_dir / "research" / "qdii_etf_intraday_momentum"
    try:
        minute_frames = []
    ...
    phases = {
        ...
        "validation": minutes.filter(
            pl.col("datetime").dt.date().is_between(
                VALIDATION_START, VALIDATION_END, closed="both"
            )
        ),
    ...
    payload = {
        "schema_version": "p0-qdii-etf-minute-data-v1",
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("/app/data/research/p0_qdii_etf_minute_data_audit.json"),
    )

"""Collect scheduled share-unlock details into complete yearly partitions."""
...
def fetch_month(
    ...
    while current <= end:
        daily = client.query(
            "share_float", {"float_date": current.strftime("%Y%m%d")}, FIELDS
        )
        if len(daily) >= ROW_LIMIT:
            raise ValueError(f"share_float hit row limit on {current}")
...
def normalize(rows: list[dict[str, Any]], year: int) -> pl.DataFrame:
    ...
    return (
        ...
        .with_columns(
            ...
            pl.col("float_date").cast(pl.Utf8).str.to_date("%Y%m%d", strict=False),
            (pl.col("float_share").cast(pl.Float64, strict=False) * 10_000.0).alias(
                "float_shares"
            ),
            pl.col("float_ratio").cast(pl.Float64, strict=False),
        )
        .drop("float_share")
        .filter(pl.col("float_date").dt.year() == year)
        .drop_nulls(["symbol", "ann_date", "float_date", "float_shares", "float_ratio"])
        .filter(
        ...
        .sort(["float_date", "symbol", "holder_name", "share_type"])
    )
...
def collect_year(client: Any, root: Path, year: int) -> dict[str, Any]:
    ...
    if frame.is_empty():
        raise ValueError(f"share_float returned no valid rows for {year}")
    ...
    return {
        ...
        "event_dates": frame.get_column("float_date").n_unique(),
        "first_float_date": frame.get_column("float_date").min(),
        "last_float_date": frame.get_column("float_date").max(),

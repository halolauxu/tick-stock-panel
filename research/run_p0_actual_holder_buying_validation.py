"""Validate the frozen strong-holder-buying direction on 2021-2023."""
...
import run_p0_actual_corporate_buying_development as development  # noqa: E402
...
def _configure_validation_period() -> None:
    ...
    development.DEVELOPMENT_END = VALIDATION_END
    development.holder.DEVELOPMENT_START = VALIDATION_START
    development.holder.DEVELOPMENT_END = VALIDATION_END
    development.repurchase.DEVELOPMENT_START = VALIDATION_START
    development.repurchase.DEVELOPMENT_END = VALIDATION_END


def evaluate_validation(payload: dict[str, Any]) -> dict[str, Any]:
    strong = payload["results"][development.HOLDER_STRONG]
    weak = payload["results"][development.HOLDER_WEAK]
    ...
    enhancement_checks = {
        ...
        "beats_weak_control_by_3pp": annualized - weak_annualized >= 0.03,
        "at_least_100_round_trips": int(strong["account"].get("trade_count") or 0) // 2
        >= 100,
        "buy_execution_at_least_90pct": strong["execution"]["buy"]["execution_rate"]
        >= 0.90,
        "sell_execution_at_least_90pct": strong["execution"]["sell"]["execution_rate"]
        >= 0.90,
        "no_unresolved_positions": strong["integrity"]["ending_unresolved_positions"] == 0,
        "cash_reconciled": strong["integrity"]["max_cash_reconciliation_error"] <= 0.01,
    }
    ...
    deploy_checks = {
        **enhancement_checks,
        ...
        >= -0.30,
        "mean_cash_ratio_at_most_60pct": float(metrics.get("mean_cash_ratio") or math.inf)
        <= 0.60,
    }
    ...
    return {
        "verdict": (
            ...
            else "TERMINATE_HOLDER_BUYING_INTENSITY"
        ),
...
def run(data_dir: Path, output: Path) -> dict[str, Any]:
    ...
    payload = development.run(data_dir, intermediate)
    payload["schema_version"] = "p0-actual-holder-buying-validation-v1"
    ...
    summary = {
        ...
        "data": payload["data"],
        "benchmark": payload["benchmark"],
        "strong_holder": payload["results"][development.HOLDER_STRONG],
        "weak_holder": payload["results"][development.HOLDER_WEAK],
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("/app/data/research/p0_actual_holder_buying_validation_v1.json"),
    )

...
**19 个公开内置策略**,每个策略一个独立 Python 文件,基于 Polars/NumPy 向量化实现(`backend/app/strategy/builtin/`):
...
| 产业链研究 | Serenity 卡点研究初筛（仅自动计算原方法论中可由本地数据证明的 36 分） |

Serenity 初筛的完整口径、已有数据映射和待采集证据见
[`serenity-bottleneck-strategy.md`](./serenity-bottleneck-strategy.md)。

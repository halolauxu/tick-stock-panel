...
系统已实现所有门槛的计算、阻断、显式发布、回滚和动态冠军账本；但下列 11 项是对真实候选的结果验收。当前没有正式研究候选，因此必须判 `FAIL`，不能用合成测试冒充 Alpha。

| ID | 需求 | 当前真实证据 | 结论 |
|---|---|---|---|
| AM-S9-001 | 13 年拼接 OOS 收益高于冠军 | 无合格数据版本、无正式候选 | FAIL |
